...
<template>
	<div v-if="page" class="prose dark:prose-invert max-w-[unset]">
		<div v-if="route.path !== '/'" class="min-h-[400px] py-0 px-2 flex flex-col justify-end bg-cover bg-no-repeat bg-center bg-blend-multiply relative -z-1 isolate" :style="{background: `linear-gradient(transparent 75%, hsl(156 7.25% 13.53% / 50%)), url(${page.image || defaultImage})`}">
...
<!--			<NuxtImg src="/img/symbol/UoM_Indig.Data.Com_Brand_Art_RGB_Symbol_Tan_tint.svg" class="absolute h-[2500px] -top-[237%] -right-[33%] opacity-30 -z-1" />-->
			<IDCLogo class="fill-white absolute h-[750px] w-auto -top-[50px] right-0 opacity-30 saturate-10 brightness-140 -z-1" />
		</div>
